...
TIFFWriteScanline(TIFF* tif, tdata_t buf, uint32 row, tsample_t sample)
{
	...
	if (strip != tif->tif_curstrip) {
		...
		{
#ifdef AUX_TIFF_FIX	//fix for random write and overlaped data
#else
...
TIFFWriteRawStrip(TIFF* tif, tstrip_t strip, tdata_t data, tsize_t cc)
{
	...
	tif->tif_curstrip = strip;
	tif->tif_row = (strip % td->td_stripsperimage) * td->td_rowsperstrip;
#ifdef AUX_TIFF_FIX	//needs for overlaped data
    if( td->td_stripbytecount[strip] > 0 )
    {
        /* this forces TIFFAppendToStrip() to do a seek */
        tif->tif_curoff = 0;
...
#ifdef AUX_TIFF_FIX	//needs for overlaped data
    if( tif->tif_dir.td_stripbytecount[tile] > 0 )
    {
        /* this forces TIFFAppendToStrip() to do a seek */
        tif->tif_curoff = 0;
    }
...
TIFFSetupStrips(TIFF* tif)
{
	...
	td->td_stripoffset = (toff_t *)
	    _TIFFmalloc(td->td_nstrips * sizeof (toff_t));
	 ...
	 */
	_TIFFmemset(td->td_stripoffset, 0, td->td_nstrips*sizeof (toff_t));
...
{
	TIFFDirectory	*td = &tif->tif_dir;
	toff_t		*new_stripoffset;
	uint32		*new_stripbytecount;

	assert(td->td_planarconfig == PLANARCONFIG_CONTIG);
	new_stripoffset = (toff_t*)_TIFFrealloc(td->td_stripoffset,
...
TIFFAppendToStrip(TIFF* tif, tstrip_t strip, tidata_t data, tsize_t cc)
{
	...
	if (td->td_stripoffset[strip] == 0 || tif->tif_curoff == 0) {
		...
		if (td->td_stripoffset[strip] != 0) {
			 ...
			 */
#ifdef AUX_TIFF_FIX	//fix for random write and overlaped data
			if ( cc > ( int )td->td_stripbytecount[strip] )
