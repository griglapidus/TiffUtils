...
#ifdef AUX_TIFF_FIX	//dir should be moved by big or regular tag size
...
_TIFFWriteDirectory(TIFF* tif, int done)
{
	 ...
	 */
#ifdef AUX_TIFF_FIX	//if data has been appended - replace existing directory link
	...
	for (fi = 0, nfi = (int)tif->tif_nfields; nfi > 0; nfi--, fi++) {
		...
		case FIELD_SUBIFD:
			...
			break;
#ifdef AUX_TIFF_GEO	//add geotiff tags
		case FIELD_MODEL_PIXEL_SCALE:
			dir->s.tdir_tag = TIFFTAG_MODEL_PIXEL_SCALE;
			dir->s.tdir_type = TIFF_DOUBLE;
			dir->s.tdir_count = td->td_nModelPixelScaleLength;
			if (!TIFFWriteDoubleArray(tif, dir, td->td_pModelPixelScale))
				goto bad;
			break;
		case FIELD_MODEL_TIE_POINT:
			dir->s.tdir_tag = TIFFTAG_MODEL_TIE_POINT;
			dir->s.tdir_type = TIFF_DOUBLE;
			dir->s.tdir_count = td->td_nModelTiePointLength;
			if (!TIFFWriteDoubleArray(tif, dir, td->td_pModelTiePoint))
				goto bad;
			break;
		case FIELD_MODEL_TRANSFORMATION:
			dir->s.tdir_tag = TIFFTAG_MODEL_TRANSFORMATION;
			dir->s.tdir_type = TIFF_DOUBLE;
			dir->s.tdir_count = td->td_nModelTransformationLength;
			if (!TIFFWriteDoubleArray(tif, dir, td->td_pModelTransformation))
				goto bad;
			break;
		case FIELD_GEO_KEY_DIRECTORY:
			dir->s.tdir_tag = TIFFTAG_GEO_KEY_DIRECTORY;
			dir->s.tdir_type = TIFF_SHORT;
			dir->s.tdir_count = td->td_nGeoKeyDirectoryLength;
			if (!TIFFWriteShortArray(tif, dir, td->td_pGeoKeyDirectory))
				goto bad;
			break;
		case FIELD_GEO_DOUBLE_PARAMS:
			dir->s.tdir_tag = TIFFTAG_GEO_DOUBLE_PARAMS;
			dir->s.tdir_type = TIFF_DOUBLE;
			dir->s.tdir_count = td->td_nGeoDoubleParamsLength;
			if (!TIFFWriteDoubleArray(tif, dir, td->td_pGeoDoubleParams))
				goto bad;
			break;
		case FIELD_GEO_ASCII_PARAMS:
			dir->s.tdir_tag = TIFFTAG_GEO_ASCII_PARAMS;
			dir->s.tdir_type = TIFF_ASCII;
			dir->s.tdir_count = td->td_nGeoAsciiParamsLength;
			if (!TIFFWriteByteArray(tif, dir, td->td_pGeoAsciiParams))
				goto bad;
			break;
		case FIELD_RPC:
			dir->s.tdir_tag = TIFFTAG_RPC;
			dir->s.tdir_type = TIFF_DOUBLE;
			dir->s.tdir_count = td->td_nRpcLength;
			if (!TIFFWriteDoubleArray(tif, dir, td->td_pRpc))
				goto bad;
			break;
#endif //AUX_TIFF_GEO
		default:
...
{
	uint32 i;
#ifdef AUX_TIFF_FIX	//offset should stay at first table (TIFFWriteData moves it)
	toff_t off = tif->tif_dataoff;
#endif //AUX_TIFF_FIX
...
TIFFWriteRationalArray(TIFF* tif, TIFFDirEntry* dir, float* v)
{
	...
	t = (uint32*) _TIFFmalloc(2 * (tsize_t)TDIRGetEntryCount(tif,dir) * sizeof (uint32));
	if (t == NULL) {
		TIFFErrorExt(tif->tif_clientdata, tif->tif_name,
...
TIFFWriteData(TIFF* tif, TIFFDirEntry* dir, char* cp)
{
	...
	cc = (tsize_t)TDIRGetEntryCount(tif,dir) * TIFFDataWidth((TIFFDataType) dir->s.tdir_type);
...
#ifdef AUX_TIFF_FIX	//if data has been appended - replace existing directory link
	toff_t prev_dir_off = tif->tif_diroff;
#endif //AUX_TIFF_FIX
...
#ifdef AUX_TIFF_FIX	//if data has been appended - replace existing directory link
		if ( diroffB == 0 )
			diroffB = ((TIFFSeekFile(tif, 0, SEEK_END) + 1) & ~(toff_t) 1);
#endif
			...
			case TIFFTAG_SUBIFD:
				...
				if (subifdcnt <= sizeof(dir->s.tdir_offset) / sizeof(uint32))
					subifdlink = diroff + sizeof(dircount) +
						(uint32)((char*) &dir->s.tdir_offset - (char*) dir);
				else
