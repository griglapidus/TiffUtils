...
#ifdef AUX_TIFF_GEO	//we don't need it
#undef HAS_FSTAT64
#undef HAS_OPEN64
#undef HAS_LSEEK64
#endif //AUX_TIFF_GEO
...
{
	return ((tsize_t) _read((int)(unsigned long long) fd, buf, (size_t) size));
}

static tsize_t
_tiffWriteProc(thandle_t fd, tdata_t buf, tsize_t size)
{
	return ((tsize_t) _write((int)(unsigned long long) fd, buf, (size_t) size));
}
...
#else
	return ((toff_t) _lseek((int)(unsigned long long) fd, (long) off, whence));
#endif
}
...
{
	return (_close((int)(unsigned long long) fd));
}
...
#else
	toff_t fsize;
	return ((fsize = _lseek((int)(unsigned long long) fd, 0, SEEK_END)) < 0 ? 0 : fsize);
#endif
}
...
TIFFFdOpen(int fd, const char* name, const char* mode)
{
	...
	tif = TIFFClientOpen(name, mode,
	    (thandle_t)(unsigned long long) fd,
