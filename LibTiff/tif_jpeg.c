...
#ifdef AUX_TIFF_FIX //it is better to read it normal
	downsampled_output = FALSE;
#endif //AUX_TIFF_FIX
    ...
    if (nrows)
    {
        ...
        */
        ...
        do {
            ...
            buf += sp->bytesperline;
            cc -= sp->bytesperline;
#ifdef AUX_TIFF_FIX //it seems as a bug
		} while (--nrows > 0 && cc >= sp->bytesperline);
		tif->tif_row--; //we realy need it
#else
		} while (--nrows > 0);
#endif //AUX_TIFF_FIX
...
#ifdef AUX_TIFF_FIX //need fix also row size
...
static int JPEGInitializeLibJPEG( TIFF * tif, int force_encode, int force_decode )
{
    ...
    if( force_decode )
        ...
        decompress = 1;
#ifdef AUX_TIFF_FIX //support overwriting to existing data
    else
        decompress = 0;
#else //AUX_TIFF_FIX
