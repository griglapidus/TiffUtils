...
static	int TIFFFetchOffsets(TIFF*, TIFFDirEntry*, long, toff_t**);
static	int TIFFFetchByteCounts(TIFF*, TIFFDirEntry*, long, uint32**);
...
TIFFReadDirectory(TIFF* tif)
{
...
#ifdef AUX_TIFF_FIX	//need to be 0 for non 64 bits
	dircount = 0;
	nextdiroff = 0;
#endif	//AUX_TIFF_FIX
	...
	if (!isMapped(tif)) {
		...
		_TIFFmemcpy(dir, tif->tif_base + off, (tsize_t)TIFFGetDirCnt(tif,dircount) * TDIREntryLen(tif));
	 ...
	 */ 
	for (dp = dir, n = (int)TIFFGetDirCnt(tif,dircount); n > 0; 
	 ...
	 */
	fix = 0;
	for (dp = dir, n = (int)TIFFGetDirCnt(tif,dircount); n > 0; n--, TDIREntryNext(tif,dp)) {
...
#ifdef AUX_TIFF_FIX	//some providers set TIFF_UNDEFINED instead of needed type
			if (fip->field_type == TIFF_ANY || dp->s.tdir_type == TIFF_UNDEFINED ||
#else
...
#ifdef AUX_TIFF_FIX	//we don't need to read it again, it causes error if big-indian, because swaps again in second pass
				dp->s.tdir_tag = IGNORE;
#endif	//AUX_TIFF_FIX
				break;
	...
	for (dp = dir, n = (int)TIFFGetDirCnt(tif,dircount); n > 0; n--, TDIREntryNext(tif,dp)) {
		...
		case TIFFTAG_STRIPOFFSETS:
		case TIFFTAG_TILEOFFSETS:
			if (!TIFFFetchOffsets(tif, dp,
			    td->td_nstrips, &td->td_stripoffset))
				goto bad;
			break;
		case TIFFTAG_STRIPBYTECOUNTS:
		case TIFFTAG_TILEBYTECOUNTS:
			if (!TIFFFetchByteCounts(tif, dp,
...
#define	BYTECOUNTLOOKSBAD \
	...
	"%s: Bogus \"%s\" field, ignoring and calculating from imagelength",
		...
		if(EstimateStripByteCounts(tif, dir, TIFFGetDirCnt(tif,dircount)) < 0)
		   ...
		   && td->td_nstrips > 2
		   && td->td_compression == COMPRESSION_NONE
#ifdef AUX_TIFF_FIX	//I saw image, that had two first - equal, but some others not
		   && td->td_stripbytecount[0] != td->td_stripbytecount[td->td_nstrips-2]) {
#else
			...
			const TIFFFieldInfo info[], size_t n)
{
	...
	if (!isMapped(tif)) {
		...
		if (off + TIFFGetDirCnt(tif,dircount) * TDIREntryLen(tif) > tif->tif_size) {
			...
			_TIFFmemcpy(dir, tif->tif_base + off,
				    (tsize_t)TIFFGetDirCnt(tif,dircount) * TDIREntryLen(tif));
		}
	}
	...
	fix = 0;
	for (dp = dir, i = (uint16)TIFFGetDirCnt(tif,dircount); i > 0; i--, TDIREntryNext(tif,dp)) {
		if (tif->tif_flags & TIFF_SWAB) {
			TIFFSwabShort(&dp->s.tdir_tag);
			TIFFSwabShort(&dp->s.tdir_type);
			TDIRSwabEntryCount(tif,dp);
...
EstimateStripByteCounts(TIFF* tif, TIFFDirEntry* dir, uint64 dircount)
{
...
#ifdef AUX_TIFF_FIX	//can has more than 65536 strips/tiles
	uint32 i;
#else
	uint16 i;
#endif	//AUX_TIFF_FIX
	...
	if (td->td_compression != COMPRESSION_NONE) {
		...
		for (dp = dir, n = dircount; n > 0; n--, TDIREntryNext(tif,dp))
		{
			...
			cc = cc * (uint32)TDIRGetEntryCount(tif,dp);
			if (cc > sizeof (uint32))
				space += cc;
		}
		...
		if (((toff_t)(td->td_stripoffset[i]+td->td_stripbytecount[i]))
			...
			td->td_stripbytecount[i] = 
			    (uint32) (filesize - td->td_stripoffset[i]);
	} else {
		uint32 rowbytes = TIFFScanlineSize(tif);
		uint32 rowsperstrip = td->td_imagelength/td->td_stripsperimage;
#ifdef AUX_TIFF_FIX	//if image is tiled - the estimated strip size is different
		uint32 size = isTiled( tif ) ? TIFFTileSize(tif) : rowbytes*rowsperstrip;
		for (i = 0; i < td->td_nstrips; i++)
			td->td_stripbytecount[i] = size;
#else
		for (i = 0; i < td->td_nstrips; i++)
...
TIFFFetchData(TIFF* tif, TIFFDirEntry* dir, char* cp)
{
	...
	tsize_t cc = (tsize_t)TDIRGetEntryCount(tif,dir) * w;
	...
	if (tif->tif_flags & TIFF_SWAB) {
		...
		case TIFF_SHORT:
		case TIFF_SSHORT:
			TIFFSwabArrayOfShort((uint16*) cp, (unsigned long)TDIRGetEntryCount(tif,dir));
			break;
		case TIFF_LONG:
		case TIFF_SLONG:
		case TIFF_IFD:
		case TIFF_FLOAT:
			TIFFSwabArrayOfLong((uint32*) cp, (unsigned long)TDIRGetEntryCount(tif,dir));
			break;
		case TIFF_LONG8:
		case TIFF_SLONG8:
		case TIFF_IFD8:
			TIFFSwabArrayOfLong8((uint64*) cp, (unsigned long)TDIRGetEntryCount(tif,dir));
			break;
		case TIFF_RATIONAL:
		case TIFF_SRATIONAL:
			TIFFSwabArrayOfLong((uint32*) cp, 2* (unsigned long)TDIRGetEntryCount(tif,dir));
			break;
		case TIFF_DOUBLE:
			TIFFSwabArrayOfDouble((double*) cp, (unsigned long)TDIRGetEntryCount(tif,dir));
			break;
		}
	}
	return (cc);
...
#ifdef AUX_TIFF_FIX	//It seems as a bug, string is 8-bit per character
			(tsize_t)TDIRGetEntryCount(tif,dir));
#else
...
TIFFFetchRational(TIFF* tif, TIFFDirEntry* dir)
{
	...
	{
		_TIFFmemcpy(&l, TDIRAddrEntryOff(tif, dir), sizeof(uint32) * 2);
		return !cvtRational(tif, dir, l[0], l[1], &v) ? 1.0f : v;
	}
	else
	{
...
TIFFFetchShortArray(TIFF* tif, TIFFDirEntry* dir, uint16* v)
{
	if (TDIRGetEntryCount(tif,dir) <= TDIREntryOffLen(tif) / sizeof(uint16)) {
		if (tif->tif_flags & TIFF_SWAB) {
			TDIRSwabEntryOff(tif,dir);
			TIFFSwabArrayOfShort(TDIRAddrEntryOff(tif,dir), 
				(unsigned long)TDIRGetEntryCount(tif,dir));
		}
		_TIFFmemcpy(v, TDIRAddrEntryOff(tif,dir),
			(tsize_t)TDIRGetEntryCount(tif,dir) * sizeof(uint16));
		return (1);
	} else
...
TIFFFetchLongArray(TIFF* tif, TIFFDirEntry* dir, uint32* v)
{
	if (TDIRGetEntryCount(tif,dir) <= TDIREntryOffLen(tif) / sizeof(uint32)) {
		...
		_TIFFmemcpy(v, TDIRAddrEntryOff(tif,dir),
			(tsize_t)TDIRGetEntryCount(tif,dir) * sizeof(uint32));
		return (1);
	} else
		return (TIFFFetchData(tif, dir, (char*) v) != 0);
}
...
TIFFFetchLong8Array(TIFF* tif, TIFFDirEntry* dir, uint64* v)
{
	if (TDIRGetEntryCount(tif,dir) <= TDIREntryOffLen(tif) / sizeof(uint64)) {
		...
		_TIFFmemcpy(v, TDIRAddrEntryOff(tif,dir),
			(tsize_t)TDIRGetEntryCount(tif,dir) * sizeof(uint64));
		return (1);
	} else
		return (TIFFFetchData(tif, dir, (char*) v) != 0);
}
...
TIFFFetchRationalArray(TIFF* tif, TIFFDirEntry* dir, float* v)
{
	...
	if (l) {
		if (TIFFFetchLongArray(tif, dir, l)) {
			uint32 i;
			uint32 count = (uint32)TDIRGetEntryCount(tif,dir) * 2;
			for (i = 0; i < count; i += 2) {
...
TIFFFetchFloatArray(TIFF* tif, TIFFDirEntry* dir, float* v)
{
	if (TDIRGetEntryCount(tif,dir) <= TDIREntryOffLen(tif) / sizeof(float)) {
		...
		_TIFFmemcpy(v, TDIRAddrEntryOff(tif,dir),
			(tsize_t)TDIRGetEntryCount(tif,dir) * sizeof(float));
...
TIFFFetchDoubleArray(TIFF* tif, TIFFDirEntry* dir, double* v)
{
	if (TDIRGetEntryCount(tif,dir) <= TDIREntryOffLen(tif) / sizeof(double)) {
		...
		_TIFFmemcpy(v, TDIRAddrEntryOff(tif,dir),
			(tsize_t)TDIRGetEntryCount(tif,dir) * sizeof(double));
	} else if (!TIFFFetchData(tif, dir, (char*) v))
		return (0);
...
TIFFFetchAnyArray(TIFF* tif, TIFFDirEntry* dir, double* v)
{
	...
	case TIFF_SBYTE:
		...
		if (dir->s.tdir_type == TIFF_BYTE) {
			uint8* vp = (uint8*) v;
			for (i = (int)TDIRGetEntryCount(tif,dir)-1; i >= 0; i--)
				v[i] = vp[i];
		} else {
			int8* vp = (int8*) v;
			for (i = (int)TDIRGetEntryCount(tif,dir)-1; i >= 0; i--)
...
TIFFFetchPerSampleShorts(TIFF* tif, TIFFDirEntry* dir, uint16* pl)
{
    ...
    if (CheckDirCount(tif, dir, samples)) {
        ...
        if (v && TIFFFetchShortArray(tif, dir, v)) {
            uint16 i;
            int check_count = (int)TDIRGetEntryCount(tif,dir);
...
/*
 * Fetch a set of offsets
 */
static int
TIFFFetchOffsets(TIFF* tif, TIFFDirEntry* dir, long nstrips, toff_t** lpp)
{
	int status;
	long tdi;
	...
	/*
	 * Allocate space for offset information.
	 */
	if (*lpp == NULL &&
	    (*lpp = (toff_t *)_TIFFCheckMalloc(tif,
	      nstrips, sizeof (toff_t), "for strip/tile offset array")) == NULL)
		return (0);
	/*
	 * Must handle cases where nstrips != TDIRGetEntryCount(tif,dir) and where array is
	 * TIFF_LONG8 (64-bit), TIFF_LONG (32-bit), or TIFF_SHORT (16-bit)
	 */
	if (dir->s.tdir_type == TIFF_LONG8) {
		toff_t *doff = (toff_t*) _TIFFCheckMalloc(tif, 
				TDIRGetEntryCount(tif,dir), sizeof(toff_t), "64-bit offset array");
		status = TIFFFetchLong8Array(tif, dir, doff);
		if (status != 0)
			for (tdi = 0; tdi < nstrips && tdi < (long) TDIRGetEntryCount(tif,dir); tdi++)
				(*lpp)[tdi] = doff[tdi];
		_TIFFfree(doff);
	} else if (dir->s.tdir_type == TIFF_LONG) {
		uint32 *doff = (uint32*) _TIFFCheckMalloc(tif, 
				TDIRGetEntryCount(tif,dir), sizeof(uint32), "32-bit offset array");
		status = TIFFFetchLongArray(tif, dir, doff);
		if (status != 0)
			for (tdi = 0; tdi < nstrips && tdi < (long) TDIRGetEntryCount(tif,dir); tdi++)
				(*lpp)[tdi] = doff[tdi];
		_TIFFfree(doff);
	} else {	/* TIFF_SHORT */
		uint16 *doff = (uint16*) _TIFFCheckMalloc(tif, 
				TDIRGetEntryCount(tif,dir), sizeof(uint16), "16-bit offset array");
		status = TIFFFetchShortArray(tif, dir, doff);
		if (status != 0)
			for (tdi = 0; tdi < nstrips && tdi < (long) TDIRGetEntryCount(tif,dir); tdi++)
				(*lpp)[tdi] = doff[tdi];
		_TIFFfree(doff);
...
/*
 * Fetch a set of byte counts
 */
static int
TIFFFetchByteCounts(TIFF* tif, TIFFDirEntry* dir, long nstrips, uint32** lpp)
{
	register uint32* lp;
	int status;
	long tdi;
	...
	/*
	 * Allocate space for byte count information.
	 */
	if (*lpp == NULL &&
	    (*lpp = (uint32 *)_TIFFCheckMalloc(tif,
	      nstrips, sizeof (uint32), "for strip/tile byte count array")) == NULL)
		return (0);
	lp = *lpp;
	 ...
	 */
	if (dir->s.tdir_type == TIFF_LONG8) {
		uint64 *dbcnt = (uint64*) _TIFFCheckMalloc(tif, 
				TDIRGetEntryCount(tif,dir), sizeof(uint64), "64-bit byte count array");
		status = TIFFFetchLong8Array(tif, dir, dbcnt);
		if (status != 0)
			for (tdi = 0; tdi < nstrips && tdi < (long) TDIRGetEntryCount(tif,dir); tdi++)
				(*lpp)[tdi] = (uint32) dbcnt[tdi];
		_TIFFfree(dbcnt);
	} else if (dir->s.tdir_type == TIFF_LONG) {
		uint32 *dbcnt = (uint32*) _TIFFCheckMalloc(tif, 
				TDIRGetEntryCount(tif,dir), sizeof(uint32), "32-bit byte count array");
		status = TIFFFetchLongArray(tif, dir, dbcnt);
		if (status != 0)
			for (tdi = 0; tdi < nstrips && tdi < (long) TDIRGetEntryCount(tif,dir); tdi++)
				(*lpp)[tdi] = dbcnt[tdi];
		_TIFFfree(dbcnt);
	} else {	/* TIFF_SHORT */
		uint16 *dbcnt = (uint16*) _TIFFCheckMalloc(tif, 
				TDIRGetEntryCount(tif,dir), sizeof(uint16), "16-bit byte count array");
		status = TIFFFetchShortArray(tif, dir, dbcnt);
		if (status != 0)
			for (tdi = 0; tdi < nstrips && tdi < (long) TDIRGetEntryCount(tif,dir); tdi++)
				(*lpp)[tdi] = dbcnt[tdi];
		_TIFFfree(dbcnt);
...
ChopUpSingleUncompressedStrip(TIFF* tif)
{
	...
	toff_t offset = td->td_stripoffset[0];
